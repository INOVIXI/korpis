...
**Status:** design
**Date:** 2026-08-07
**Depends on:** [`01-model.md`](./01-model.md), [`04-runtimes.md`](./04-runtimes.md), [`05-scheduling.md`](./05-scheduling.md)
**Implements:** Rules K-3, K-5, K-15

---

## 1. One decision, four consequences

Rule K-5 says backup is a consequence of the storage design rather than a feature on top of it. That
is the single most useful idea available in this space, and it is worth stating precisely, because
it determines four things at once:
...
whatever the node's root filesystem happens to be. Every downstream consequence follows: backups are
full `tar.gz` archives every time (#5493 asks for incremental, still open); a 40 GB server means
writing 40 GB, unthrottled, killing the node's iowait for everyone (#2798); backups stage to local
disk before upload, so the node needs free space equal to the backup (#3846); S3 uploads fail above
5 GB because multipart is not used (#2599); disk usage is computed by walking the directory tree, so
the panel misreports it (#5186); and the quota is advisory, so an 858 GB file can exist on a server
with a 1 GB limit (#4554, closed as *not planned*).
...
| **Network (NFS/iSCSI/Ceph)** | backend-dependent | backend-dependent | backend-dependent | — | — | required for `shared_access` |
...
above all **advisory**: nothing stops the write. The limit is a number in a database compared against
a number produced by a background process. Neither is in the write path.
...
3. **Inode quotas exist.** A tenant can exhaust a filesystem with millions of empty files without
   using meaningful capacity. `quota_dimensions` includes inodes, and both are enforced. No panel in
...
spinning disk cause iowait spikes that degrade every server on the node. In Korpis the backup worker
runs in its own cgroup with a low `io.weight` and explicit ceilings, so it consumes only capacity
tenants are not using. Backup never competes with the workloads it is protecting.
...
A snapshot is a cheap local rollback point and the mechanism that makes backup non-disruptive. It is
not protection against losing the node. Korpis names them differently and never presents a snapshot
as a backup.
...
| Deduplicated | content-addressed chunks | — |
| Encrypted before leaving the node | client-side, per-repository key | — |
| Single-file restore | the manifest is a tree; fetch only the needed chunks | — |
...
manifest is a tree of chunk references, restoring one config file from three weeks ago fetches a few
kilobytes. Pterodactyl requires downloading and extracting the entire `tar.gz`.
...
disaster on top of the first. The safe operation is the prominent one; the destructive one is marked
irreversible and requires approval.
...
- **`shared_access` volumes carry the `on_expiry: stop` lease policy by default** (§4.5 of
  ...
  shared storage must stop when its lease cannot be renewed. Local volumes default to `keep_running`,
  because nothing else can reach that disk.
...
1. **Encryption key custody.** Client-side encryption means a lost key is lost data, and a key
   ...
   ceremony, KMS integration, or per-organization keys wrapped by an operator key. The last supports
   multi-tenancy but makes the operator able to read tenant backups — which may be exactly right for
   a hosting provider and exactly wrong for a shared community. → here, and `17-security.md`
...
3. **Network storage backends.** `shared_access` is required for VM live migration and for failover,
   but Ceph, NFS, and iSCSI have very different consistency, performance, and failure characteristics.
   Whether one abstraction covers them or each needs its own class is unresolved. → here
...
5. **Cross-node deduplication.** Dedup is currently per-repository, which means a hundred nodes
   ...
   chunk index across nodes would fix it and reintroduces the confirmation-of-file oracle of §5.3 at
   a larger scale. → here
